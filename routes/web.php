	...
	*/
	Route::group(['middleware' => ['get.menu', 'auth']], function () {
		...
		Route::group(['namespace' => 'Projects', 'middleware' => []], function () {
			Route::get('projects/chart', 'ProjectController@chart')->name('projects.chart');
			Route::get('projects/{project}/status/{status}', 'ProjectController@status')->name('projects.status');
		...
		Route::group(['namespace' => 'Accounting', 'prefix' => 'accounting', 'as' => 'accounting.', 'middleware' => []], function () {
			...
			// Custom Routes
			Route::get('chartOfAccounts/{account}/activate', 'ChartOfAccountController@activate')->name('chartOfAccounts.activate');
			Route::get('chartOfAccounts/{account}/deactivate', 'ChartOfAccountController@deactivate')->name('chartOfAccounts.deactivate');
			Route::get('chartOfAccounts/{account}/register', 'ChartOfAccountController@register')->name('chartOfAccounts.register');
			Route::put('chartOfAccounts/{account}/register', 'ChartOfAccountController@register_update')->name('chartOfAccounts.register.update');
		});
		
		Route::group(['namespace' => 'Search', 'prefix' => 'search', 'as' => 'search.', 'middleware' => []], function () {
			Route::post('submit', 'SearchController@index')->name('submit');
		});
	});
	Auth::routes();